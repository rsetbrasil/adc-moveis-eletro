...
async function main() {
    try {
        console.log('🚀 Starting Migration from Supabase to MySQL VPS...');
        console.log(`📍 Target Database: ${process.env.DATABASE_URL?.split('@')[1]?.split('/')[1] || 'Unknown'}`);
        ...
        await migrateTable('commission_payments', prisma.commissionPayment, (row) => ({
            id: row.id,
            sellerId: row.seller_id ?? row.sellerId,
            sellerName: row.seller_name ?? row.sellerName,
            amount: row.amount,
            ...
            period: row.period,
            orderIds: deepNormalizeKeys(row.order_ids ?? row.orderIds),
        }));

        // 7. Stock Audits
        await migrateTable('stock_audits', prisma.stockAudit, (row) => ({
            id: row.id,
            month: row.month,
            year: row.year,
            createdAt: row.created_at ? new Date(row.created_at) : undefined,
            auditedBy: row.audited_by ?? row.auditedBy,
            auditedByName: row.audited_by_name ?? row.auditedByName,
            products: deepNormalizeKeys(row.products),
        }));

        // 8. Avarias
        await migrateTable('avarias', prisma.avaria, (row) => ({
            id: row.id,
            createdAt: row.created_at ? new Date(row.created_at) : undefined,
        ...
        // 9. Chat Sessions
        await migrateTable('chat_sessions', prisma.chatSession, (row) => ({
            id: row.id,
            ...
            sellerId: row.seller_id ?? row.sellerId,
            sellerName: row.seller_name ?? row.sellerName,
            unreadBySeller: row.unread_by_seller ?? row.unreadBySeller,
            unreadByVisitor: row.unread_by_visitor ?? row.unreadByVisitor,
            satisfaction: row.satisfaction,
        }));

        // 10. Chat Messages
        await migrateTable('chat_messages', prisma.chatMessage, (row) => ({
            id: row.id,
            ...
            attachment: deepNormalizeKeys(row.attachment),
            type: row.type,
            rating: row.rating,
        }));
        ...
        console.log('\n🎉 Migration Complete!');
        console.log('\n📊 Summary:');
        console.log('   ✅ All tables migrated from Supabase to MySQL VPS');
        console.log('   ✅ Data normalized and transformed');
